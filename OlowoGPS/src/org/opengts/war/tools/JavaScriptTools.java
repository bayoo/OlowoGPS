...
public class JavaScriptTools
{
    ...
    public static void writeJSInclude(PrintWriter out, String jsFileRef)
        throws IOException
    {
        out.write("<script src=\""+jsFileRef+"\" type=\"text/javascript\"></script>\n");
    }
    
    public static void writeJSIncludes(PrintWriter out, String jsFileRefs[]) 
        throws IOException
    {
        if (jsFileRefs != null) {
            for (int i = 0; i < jsFileRefs.length; i++) {
                ...
                if (!StringTools.isBlank(js)) {
                    JavaScriptTools.writeJSInclude(out, js);
                }
            }
        }
    }
    ...
    public static void writeUtilsJS(PrintWriter out)
        throws IOException
    {
        JavaScriptTools.writeJSInclude(out, JavaScriptTools.qualifyJSFileRef("utils.js"));
    }
