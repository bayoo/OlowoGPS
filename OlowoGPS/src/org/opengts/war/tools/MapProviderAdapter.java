    ...
    implements MapProvider
{
    ...
    **/
    ...
    /**
    *** Writes the events in XML format to the http output stream.  The criteria used for
    ...
    **/
        ...
        throws IOException
    {
        ...
        /* write map data */
        this.writeMapUpdate(out, 0, reqState, statusCodes);

    }

    /**
    *** Writes the events in CSV format to the http output stream.  The criteria used for
    *** selecting EventData records is specified with the RequestProperties session state.
        ...
        try {
            ...
            if (RTConfig.isDebugMode()) {
                evUtil.writeMapEvents(
                    indentLevel, new PrintWriter(System.out,true),
                    privLabel, 
                    evdata, inclZones,
                    iconSel, iconKeys,
                    isFleet, fleetRoute, selID,
                    tmz,acct,latest,lastBatt,lastSig,proximityM);
            }
            / * */
            evUtil.writeMapEvents(
                indentLevel, out, reqState.isSoapRequest(),
                privLabel, 
                evdata, inclZones,
                iconSel,iconKeys,
                isFleet, fleetRoute, selID,
