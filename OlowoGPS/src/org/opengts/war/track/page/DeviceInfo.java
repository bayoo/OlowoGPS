    ...
    */
    ...
    private int _allowNewDevice(Account currAcct, PrivateLabel privLabel)
    {
        ...
        if (Account.isSystemAdmin(currAcct)) {
            ...
            if (currAcct.exceedsMaximumDevices(currAcct.getDeviceCount() + 1)) {
                // deny if over limit
    ...
    private String _updateDeviceTable(RequestProperties reqState)
    {
      ...
      //String  worderID  = AttributeTools.getRequestString(request, PARM_WORKORDER_ID      , "");
        try {
            ...
            // server id
            boolean editServID = EDIT_SERVER_ID && (selDev.getLastConnectTime() <= 0L);
        ...
        throws IOException
    {
        ...
        /* "Properties" */
        boolean allowProp    = allowView && privLabel.hasWriteAccess(currUser, this.getAclName(_ACL_COMMANDS)) &&
      ...
      //boolean refreshList  = deviceCmd.equals(COMMAND_INFO_REFRESH);
        ...
        final boolean _uiList      = uiList || (!_uiEdit && !_uiView && !_uiProp && !_uiSms);
                    ...
                    for (int d = 0; d < _deviceList.size(); d++) {
                        ...
                        try {
                            ...
                            if (dev != null) {
                                ...
                                if (DeviceInfo.this.getDeviceCommandHandler(dev.getDeviceCode()) != null) {
                                    hasProperties++;
                                }
                            }
                        } catch (DBException dbe) {
        ...
        if (_uiEdit || _uiView) {
            ...
            HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
                public void write(PrintWriter out) throws IOException {
                    ...
                    /* Device fields */
                    boolean edSrvID = (EDIT_SERVER_ID && (_selDev != null) && (_selDev.getLastConnectTime() <= 0L)); // not yet connected
        ...
        String onload = (_uiProp || _uiSms)? "javascript:devCommandOnLoad();" : onloadAlert;
