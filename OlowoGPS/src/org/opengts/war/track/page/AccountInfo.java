    ...
    implements Constants
{
    ...
    public String getNavigationDescription(RequestProperties reqState)
    {
        PrivateLabel privLabel = reqState.getPrivateLabel();
        I18N i18n = privLabel.getI18N(AccountInfo.class);
        return super._getNavigationDescription(reqState,i18n.getString("AccountInfo.navDesc","Account Admin"));
    }
            ...
            if (SubmitMatch(submit,i18n.getString("AccountInfo.change","Change"))) {
                ...
                try {
                  ...
                  //currAcct.setGeocoderMode(revGeoMode, locale);
                    // 'Device' title
                    //int devSep = deviceTitle.indexOf("|");
                    String devSingTitle = deviceTitle;  // (devSep >= 0)? deviceTitle.substring(0,devSep).trim() : deviceTitle;
                    String devPlurTitle = devicesTitle; // (devSep >= 0)? deviceTitle.substring(devSep+1).trim() : deviceTitle;
                    currAcct.setDeviceTitle(devSingTitle, devPlurTitle);
                    // 'DeviceGroup' title
                    //int grpSep = groupTitle.indexOf("|");
                    String grpSingTitle = groupTitle;  // (grpSep >= 0)? groupTitle.substring(0,grpSep).trim() : groupTitle;
                    String grpPlurTitle = groupsTitle; // (grpSep >= 0)? groupTitle.substring(grpSep+1).trim() : groupTitle;
        ...
        HTMLOutput HTML_CONTENT  = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
            public void write(PrintWriter out) throws IOException {
              ...
              //String chgURL  = EncodeMakeURL(reqState,RequestProperties.TRACK_BASE_URI(),pageName,COMMAND_INFO_UPDATE);
                ...
                String expireTimeStr = reqState.formatDateTime(currAcct.getExpirationTime());
              ...
              //ComboOption geocoderMode     = privLabel.getEnumComboOption(Account.getGeocoderMode(currAcct)    );
                ...
                out.print  ("</td>");
                out.println("</tr>");
              //out.println(FormRow_TextField(PARM_DEVICE_TITLE , _allowEdit, i18n.getString("AccountInfo.deviceTitle","'Device' Title:")            , devTitles[0]                  , 20, 40));
                ...
                out.print  ("</td>");
                out.println("</tr>");
              //out.println(FormRow_TextField(PARM_GROUP_TITLE  , _allowEdit, i18n.getString("AccountInfo.groupTitle","'DeviceGroup' Title:")        , grpTitles[0]                  , 20, 40));
