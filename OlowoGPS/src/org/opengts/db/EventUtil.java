...
public class EventUtil
{
    ...
    **/
        ...
        throws IOException
    {
        ...
        return writeEvents_CSV(pwout, 
            account, devList, allTags, tmzone, 
            csvSep, inclHeader, privLabel);
    }
    
    private boolean writeEvents_CSV(PrintWriter pwout, 
        Account account, Collection<Device> devList, boolean allTags, TimeZone tz, 
        ...
        throws IOException
    {
        ...
        return this.writeEvents_CSV(pwout, 
            account, devList, evFields, tz, 
            csvSep, inclHeader, privLabel);

    }
    
    private boolean writeEvents_CSV(PrintWriter pwout, 
        Account account, Collection<Device> devList, String evFields[], TimeZone tz, 
        char csvSep, boolean inclHeader, BasicPrivateLabel privLabel)
        throws IOException
    {
        ...
        if (!ListTools.isEmpty(devList)) {
            for (Device dev : devList) {
                ...
                for (EventData ev : evList) {
                    ...
                    /* write event */
                    String rcd = this.formatEventData_CSV(ev, evFields, tz, dateFmt, timeFmt, csvSep) + "\n";
  ...
  //public  static final String  CSV_SEPARATOR                  = "|";
    ...
    public String getParseMapEventJS(boolean isFleet, Locale locale, char csvSep)
    {
        ...
        js.append("    this.address   = (fld.length > 16)? fld[16].trim() : '';\n");
    ...
    {
        // ID|DeviceDesc|Epoch|Date|Time|Timezone|StatusCode|Icon|Latitude|Longitude|Accuracy|Satellites|SpeedKPH|Heading|Altitude|Address
        ...
        GeoPoint bgp = edp.getBestGeoPoint();
        sb.append(StringTools.format(bgp.getLatitude() ,"0.000000"));
        sb.append(csvSep);
        sb.append(StringTools.format(bgp.getLongitude(),"0.000000"));
        sb.append(csvSep);

        /* [10] Accuracy (meters) */
        //Print.logInfo("GeoPoint: " + bgp + " [accuracy " + edp.getBestAccuracy() + " meters]");
        sb.append(StringTools.format(edp.getBestAccuracy(),"0.0"));
        sb.append(csvSep);
  ...
  //public  static final String  ATTR_timezone                  = "timezone";
    ...
    private void writeEvents_XML_Event(PrintWriter pwout, 
        ...
        int indent, boolean allTags, BasicPrivateLabel privLabel, boolean oldFormat)
        throws IOException
    {
    ...
    private boolean writeEvents_XML(PrintWriter pwout, 
        Account account, Collection<Device> devList, boolean allTags, 
        ...
        throws IOException
    {
        ...
        if (allTags || (timestamp > 0L)) {
            TimeZone tz = account.getTimeZone(null);
            DateTime ts = new DateTime(timestamp); // 'tz' used below
            this.writeKeyValue_JSON(pwout,PFX2,"Timestamp", String.valueOf(timestamp), false);
        ...
        String code = "0x" + StringTools.toHexString(statusCode, 16);
        String desc = ev.getStatusCodeDescription(privLabel);
        this.writeKeyValue_JSON(pwout,PFX2,"StatusCode", code, false);
        ...
        if (allTags || geoPoint.isValid()) {
            ...
            this.writeKeyValue_JSON(pwout,PFX2,"GPSPoint_lat", StringTools.format(geoPoint.getLatitude(),"0.00000"), false);
            this.writeKeyValue_JSON(pwout,PFX2,"GPSPoint_lon", StringTools.format(geoPoint.getLongitude(),"0.00000"), false);
            if (gpsAge > 0) {
                this.writeKeyValue_JSON(pwout,PFX2,"GPSPoint_age", String.valueOf(gpsAge), false);
            }
            if (accuracy > 0) {
                this.writeKeyValue_JSON(pwout,PFX2,"GPSPoint_accuracy", String.valueOf(accuracy), false);
            }
        }
        ...
        if (allTags || (speedKPH >= 0.0)) {
            ...
            double speedLimKPH = ev.getSpeedLimitKPH();
            double limit = (speedLimKPH > 0.0)? Account.getSpeedUnits(account).convertFromKPH(speedLimKPH) : 0.0;
            this.writeKeyValue_JSON(pwout,PFX2,"Speed", StringTools.format(speed,"0.0"), false);
            this.writeKeyValue_JSON(pwout,PFX2,"Speed_units", units, false);
            if (limit>0.0) { this.writeKeyValue_JSON(pwout,PFX2,"Speed_limit", StringTools.format(limit,"0.0"), false); }
        }
        
        // Heading (only if speed is > 0)
        if (allTags || (speedKPH > 0.0)) {
            this.writeKeyValue_JSON(pwout,PFX2,"Heading", StringTools.format(ev.getHeading(),"0.0"), false);
            this.writeKeyValue_JSON(pwout,PFX2,"Heading_desc", GeoPoint.GetHeadingString(ev.getHeading(),locale), false);
        }
        ...
        if (allTags || (altitudeM > 0.0)) {
            ...
            this.writeKeyValue_JSON(pwout,PFX2,"Altitude", StringTools.format(altitude,"#0"), false);
        ...
        if (allTags || (odomKM > 0.0)) {
            double odometer = Account.getDistanceUnits(account).convertFromKM(odomKM);
            String units    = Account.getDistanceUnits(account).toString(locale);
            this.writeKeyValue_JSON(pwout,PFX2,"Odometer", StringTools.format(odometer,"#0.0"), false);
        ...
        if (allTags || !geozoneID.equals("") || (geozoneNdx > 0L)) {
            ...
            this.writeKeyValue_JSON(pwout,PFX2,"Geozone_index", String.valueOf(geozoneNdx), false);
        }
        ...
        long inputMask = ev.getInputMask();
        if (allTags || (inputMask != 0L)) {
            String hexStr = "0x" + StringTools.toHexString(inputMask);
            this.writeKeyValue_JSON(pwout,PFX2,"DigitalInputMask", hexStr, false);
        }
        ...
        if (allTags && EventData.getFactory().hasField(EventData.FLD_engineRpm)) {
            long engineRpm = ev.getEngineRpm();
            this.writeKeyValue_JSON(pwout,PFX2,"EngineRPM", String.valueOf(engineRpm), false);
        }

        // [allTags] EngineHours
        if (allTags && EventData.getFactory().hasField(EventData.FLD_engineHours)) {
            double engineHours = ev.getEngineHours();
            this.writeKeyValue_JSON(pwout,PFX2,"EngineHours", StringTools.format(engineHours,"#0.0"), false);
        }

        // [allTags] VehicleBatteryVolts
        if (allTags && EventData.getFactory().hasField(EventData.FLD_vBatteryVolts)) {
            double battVolts = ev.getVBatteryVolts();
            this.writeKeyValue_JSON(pwout,PFX2,"VehicleBatteryVolts", StringTools.format(battVolts,"#0.0"), false);
        }
        ...
        if (allTags && EventData.getFactory().hasField(EventData.FLD_coolantLevel)) {
            ...
            this.writeKeyValue_JSON(pwout,PFX2,"EngineCoolantLevel", StringTools.format(pct100,"#0.0"), false);
        ...
        if (allTags && EventData.getFactory().hasField(EventData.FLD_coolantTemp)) {
            ...
            double tempC = ev.getCoolantTemp();
            double temp  = tempUnits.convertFromC(tempC);
            this.writeKeyValue_JSON(pwout,PFX2,"EngineCoolantTemperature", StringTools.format(temp,"#0.0"), false);
            this.writeKeyValue_JSON(pwout,PFX2,"EngineCoolantTemperature_units", tempUnits.toString(), false);
        }
        ...
        if (allTags && EventData.getFactory().hasField(EventData.FLD_fuelTotal)) {
            ...
            double fuelL = ev.getFuelTotal();
            double fuel  = volUnits.convertFromLiters(fuelL);
            this.writeKeyValue_JSON(pwout,PFX2,"EngineFuelUsed", StringTools.format(fuel,"#0.0"), false);
            this.writeKeyValue_JSON(pwout,PFX2,"EngineFuelUsed_units", volUnits.toString(), false);
        }

        /* index */
        this.writeKeyValue_JSON(pwout,PFX2,"Index", String.valueOf(index), true);
        ...
        throws IOException
    {
        ...
        if ("]".equals(key)) {
            ...
            // property definition
            this.write(pwout, pfx + "\"" + key + "\": \"" + StringTools.escapeJSON(value) + "\"");
            if (isLast) {
                this.write(pwout, "\n");
            } else {
                this.write(pwout, ",\n");
        ...
        throws IOException
    {
        if (devList != null) {
            switch (formatEnum) {
                ...
                case FORMAT_CSV:
                    return this.writeEvents_CSV(pwout, 
                        account, devList, allTags, null/*TimeZone*/, 
